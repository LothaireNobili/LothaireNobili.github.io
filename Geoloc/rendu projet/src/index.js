...
const renderer = new WebGLRenderer({ antialias: true, alpha: true }); //alpha : allow transparence
...
(async function () {
	...
	let envmapTexture = await new RGBELoader()
	  ...
	  .loadAsync("assets/old_room_2k.hdr");  // source : https://polyhaven.com/hdris
  ...
  let textures = {
    // source : https://free3d.com/user/ali_alkendi
  ...
  let planesData = [
    ...
    makePlane(plane, textures.planeTrailMask, envMap, scene),
	makePlane(plane, textures.planeTrailMask, envMap, scene)
  ]; //penser à optimiser si on a le temps
  ...
  let sphere = new Mesh(
    ...
    new MeshPhysicalMaterial({
          ...
          bumpMap: textures.bump,  //volume
          bumpScale: 0.3,
          envMap,
          envMapIntensity: 0.3,
          sheen: 1,  //to have a aura, looks cozy
          sheenRoughness: 0.5,
          sheenColor: new Color("#ff8a00").convertSRGBToLinear(), //color of the aura
          clearcoat: 0.5,
    })
  );
  ...
  window.addEventListener("mousemove", (e) => {
    ...
    anime({
      ...
      update: () => {
        ...
        scene.children.forEach((child) => {   //that whole thing : change the opacity of the rings in DN cycles
  ...
  renderer.setAnimationLoop(() => {
	...
	planesData.forEach(planeData => {  //make plane moves
    ...
    renderer.autoClear = false; //not clearing
    renderer.render(ringsScene, ringsCamera);  //requiered cuz background cleaning bc of other rendered
    renderer.autoClear = true; //clearing again
  });
})();
...
function makePlane(planeMesh, trailTexture, envMap, scene) {
	...
	plane.position.set(0,0,0);
	plane.rotation.set(0,0,0);
	plane.updateMatrixWorld(); //those line set size+pos correctly
  ...
  function nr() {
	return Math.random() * 2 - 1;  //btw -1 and 1
  }